...
def audit_html_semantics(html_templates_path):
    ...
    for html_file in html_files:
        with open(html_file, "r", encoding="utf-8") as f:
            ...
            for img in soup.find_all("img"):
                if img.get("alt") is None:
                    issues["images_without_alt"].append(
                        f"{html_file}: <img src='{img.get('src', 'N/A')}' missing alt attribute>"
                    )

            # --- Check 2: Links without href attribute ---
            for a in soup.find_all("a"):
                if (
                    not a.get("href") and a.text.strip()
                ):  # Only report if it has content but no href
                    issues["links_without_href"].append(
                        f"{html_file}: <a> tag with text '{a.text.strip()}' missing href attribute"
                    )

            # --- Check 3: Buttons without type attribute ---
            for button in soup.find_all("button"):
                if not button.get("type"):
                    issues["buttons_without_type"].append(
                        f"{html_file}: <button> tag with text '{button.text.strip()}' missing type attribute"
                    )

            # --- Check 4: Elements with positive tabindex ---
            for el in soup.find_all(tabindex=re.compile(r"^[1-9]\d*$")):  # tabindex > 0
                issues["elements_with_positive_tabindex"].append(
                    f"{html_file}: <{el.name}> tag with positive tabindex='{el.get('tabindex')}' found. Prefer tabindex='0' or none."
                )
            ...
            last_level = 0
            for level, h_element in sorted(
                headings, key=lambda x: str(x[1])
            ):  # Sort by element's string representation for basic ordering
                if last_level == 0:  # First heading found
                    last_level = level
                    continue
                if level > last_level + 1:
                    issues["heading_level_skips"].append(
                        f"{html_file}: Heading level skipped from H{last_level} to H{level} for text '{h_element.text.strip()}'"
                    )
                last_level = level

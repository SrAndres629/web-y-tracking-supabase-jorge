    ...
    def validate_critical(self):
        ...
        if not self.DATABASE_URL:
            ...
            if is_prod and ":5432" in self.DATABASE_URL:
                logger.warning(
                    "🔥 CRITICAL ARCHITECTURE WARNING: You are using Port 5432 (Session Mode) in Serverless."
                )
                logger.warning(
                    "👉 PLEASE SWITCH TO PORT 6543 (Transaction Pooler) to avoid connection limits."
                )

            if is_prod and "pgbouncer=true" not in self.DATABASE_URL:
                logger.warning(
                    "⚠️ PERFORMANCE TIP: Add '?pgbouncer=true' to your DATABASE_URL for stability."
                )

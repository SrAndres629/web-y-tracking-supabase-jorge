...
def test_vercel_include_files_physical_contract():
    ...
    # Vercel supports curly brace expansion in globs sometimes, 
    # but for this test we'll handle the common patterns we use.
    # Pattern: {api/templates/**,static/**}
    
    import re
    # Simple expansion for {a,b}
    match = re.match(r"\{(.*)\}", include_files)
    if match:
    ...
    for pattern in patterns:
        # Convert vercel glob ** to something python glob likes
        search_pattern = str(PROJECT_ROOT / pattern.replace("**", "*"))
        
        matches = glob.glob(search_pattern, recursive=True)
        assert len(matches) > 0, f"🔥 Glob '{pattern}' (expanded as '{search_pattern}') matched ZERO files. Vercel deployment will miss these!"

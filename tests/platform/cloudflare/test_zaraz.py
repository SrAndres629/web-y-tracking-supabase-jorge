...
class TestZarazFacebookPixel:
    ...
    def test_facebook_pixel_exists(self):
        ...
        fb_tools = {k: v for k, v in tools.items() if v.get("component") == "facebook-pixel"}
    ...
    def test_facebook_pixel_enabled(self):
        ...
        for tool_id, tool in tools.items():
            if tool.get("component") == "facebook-pixel":
                assert tool.get("enabled") is True, f"Facebook Pixel tool '{tool_id}' is disabled"
    ...
    def test_ecommerce_enabled(self):
        ...
        for _tool_id, tool in tools.items():
            if tool.get("component") == "facebook-pixel":
                ...
                assert ecommerce is True, "Ecommerce tracking is disabled on the Facebook Pixel"
...
class TestZarazActions:
    ...
    def test_pageview_action_exists(self):
        ...
        for _tool_id, tool in tools.items():
            if tool.get("component") == "facebook-pixel":
                ...
                pageview_actions = {
                    k: v for k, v in actions.items() if v.get("actionType") == "pageview"
                }
                assert len(pageview_actions) > 0, "No PageView action found on Facebook Pixel"
    ...
    def test_custom_event_action_exists(self):
        ...
        for _tool_id, tool in tools.items():
            if tool.get("component") == "facebook-pixel":
                actions = tool.get("actions", {})
                event_actions = {k: v for k, v in actions.items() if v.get("actionType") == "event"}
                assert len(event_actions) > 0, "No custom event action found on Facebook Pixel"
...
class TestZarazTriggers:
    ...
    def test_all_tracks_has_exclusion_rules(self):
        ...
        internal_exclusion = any(r.get("value", "").startswith("^__zaraz") for r in load_rules)
        assert internal_exclusion, "AllTracks trigger does not exclude internal __zaraz* events"

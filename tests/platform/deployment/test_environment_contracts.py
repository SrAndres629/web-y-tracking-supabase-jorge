...
def test_database_url_alias_contract():
    ...
    assert settings.db.is_configured is True, (
        "🔥 CONTRACT BREACH: Database should be marked as configured when URL is present."
    )
...
def test_critical_env_visibility():
    ...
    """
    critical_vars = {"META_PIXEL_ID": "123456", "META_ACCESS_TOKEN": "token_abc"}
    ...
    assert settings.meta.pixel_id == "123456", "🔥 FAILED: META_PIXEL_ID not loaded. Check alias."
    assert settings.meta.access_token == "token_abc", (
        "🔥 FAILED: META_ACCESS_TOKEN not loaded. Check alias."
    )
...
def test_vercel_fail_fast_contract():
    ...
    with pytest.raises(RuntimeError) as excinfo:
        ...
        _ = Database(settings=mock_settings)

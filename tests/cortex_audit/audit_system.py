...
def audit_templates():
    ...
    if missing:
        logger.error("❌ BROKEN ROUTES / INCLUDES:")
        for m in missing:
            logger.error(f"   - {m}")
    else:
    ...
    if orphaned:
        logger.warning("⚠️ ORPHANED TEMPLATES (Not reachable from Python routes):")
        for o in orphaned:
            logger.warning(f"   - {o}")
    else:
...
def audit_config():
    ...
    try:
        ...
        if settings.META_PIXEL_ID and not settings.META_API_VERSION:
            issues.append(
                "META_PIXEL_ID is set but META_API_VERSION is missing (defaulting? validation needed)."
            )
        ...
        logger.exception("❌ CRITICAL: Could not import app.config")
        return False
    return True

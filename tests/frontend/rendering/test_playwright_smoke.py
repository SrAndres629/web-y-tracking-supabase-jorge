...
@pytest.mark.skipif(os.getenv("CI") is None and os.getenv("SMOKE_TEST") is None, 
...
def test_production_smoke_200():
    ...
    with sync_playwright() as p:
        ...
        # 1. Check HTTP Status
        assert response.status == 200, f"🔥 Smoke Test Failed: {target_url} returned {response.status}"
        
        # 2. Check for critical content (Hero section)
        # Assuming "30 AÑOS" or "Jorge Aguirre Flores" is always there
        content = page.content()
        assert "Jorge Aguirre Flores" in content, "🔥 Smoke Test Failed: Critical branding missing from page."

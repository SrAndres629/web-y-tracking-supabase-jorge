...
def test_bot_detection_identifies_bots(middleware, user_agent):
    """
    Test that the bot detection regex correctly identifies common bots.
    """
    assert middleware.bot_pattern.search(user_agent), f"Failed to identify bot: {user_agent}"

@pytest.mark.parametrize("user_agent", HUMAN_USER_AGENTS)
def test_bot_detection_ignores_humans(middleware, user_agent):
    """
    Test that the bot detection regex does not incorrectly identify human user agents as bots.
    """
    assert not middleware.bot_pattern.search(user_agent), f"Incorrectly identified human as bot: {user_agent}"

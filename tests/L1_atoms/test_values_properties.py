...
class TestExternalIdProperties:
    ...
    def test_external_id_different_inputs_produce_different_ids(self, ip1, ua1, ip2, ua2):
        """
    ...
    def test_external_id_from_invalid_string_always_fails(self, invalid_string):
        ...
        assume(
            len(invalid_string) != 32
            or not all(c in "0123456789abcdef" for c in invalid_string.lower())
        )
...
class TestPhoneProperties:
    ...
    @given(
        st.text(alphabet="123456789", min_size=7, max_size=8),
        st.text(alphabet="123456789", min_size=7, max_size=8),
    )

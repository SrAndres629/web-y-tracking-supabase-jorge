...
# This test suite ONLY runs if AUDIT_MODE=1 is set.
# It proves that the system can connect to the REAL world.

@pytest.mark.skipif(os.getenv("AUDIT_MODE", "").strip() != "1", reason="Skipping Strict Integration tests (AUDIT_MODE not set)")
@pytest.mark.asyncio
async def test_real_redis_connection():
    ...
    # Use the synchronous Redis URL for connection test if available, or construct it
    # Note: app.main uses a specific client, here we test connectivity generally
    # Assuming REDIS_URL is the connection string
...
def test_real_database_connection():
    ...
    try:
        # Use the app's database utility

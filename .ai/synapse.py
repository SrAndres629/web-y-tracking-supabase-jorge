...
# 🧠 AGENT PROTOCOLS (Using venv-aware relative commands)
AGENTS = {
...
# 📜 HIVE MIND PROTOCOL (The "System Prompt")
HIVE_MIND_PROTOCOL = """
...
def update_visual_cortex():
    ...
    try:
        ...
        log("Visual Cortex updated.", "👁️")
    except Exception as e:
        log(f"Visual Cortex update failed: {e}", "⚠️")
...
def save_sensory_trace(agent_name, cmd, exit_code, stdout, stderr):
    try:
        ...
        log(f"Sensory Trace captured: {sensory_file}", "📥")
        
        if exit_code != 0:
             print(f"   >> STDERR: {stderr[:200]}...")

    except Exception as e:
        log(f"Sensory Save Failed: {e}", "🔥")
...
def execute_agent(agent_name, task_file):
    log(f"Triggering Agent: {agent_name.upper()}", "⚡")
    ...
    try:
        ...
        if len(final_task_content) > 30000:
            log(f"⚠️ Task content too large ({len(final_task_content)} chars). Truncating...", "✂️")
            final_task_content = final_task_content[:30000]

        full_command = [cmd_path] + prefix_args + [final_task_content]
        
        log(f"   >> Executing: {os.path.basename(cmd_path)} (Args: {prefix_args})", "⚙️")
        ...
        if return_code != 0:
            log(f"Agent process exited with code {return_code}", "⚠️")
            return False
            
        log(f"Agent process completed successfully.", "✅")
        
        # 👁️ VISUAL CORTEX UPDATE (Post-Task Hook)
        ...
        if agent_name in AGENTS:
            ...
            if success:
                ...
                if retries < MAX_RETRIES:
                    ...
                    log(f"🔄 RETRY {task_retries[filename]}/{MAX_RETRIES}...", "🩹")
                    continue
...
def main():
    log("Neural Network Online. Monitoring Motor Cortex...", "🌐")
    for d in [MOTOR_DIR, SENSORY_DIR, SIGNALS_DIR, MEMORY_DIR]:
        if not os.path.exists(d): os.makedirs(d)
    print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] 🌐 [SYNAPSE] Neural Network Online. Waiting for tasks in .ai/motor/...", end="", flush=True)
    ...
    while True:
        try:
            ...
            log(f"CRITICAL SYNAPSE ERROR: {e}", "🔥")
            time.sleep(5)

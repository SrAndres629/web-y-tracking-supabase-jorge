...
class PostgreSQLEventRepository(EventRepository):
        ...
        try:
            async with db.connection() as conn:
                ...
                if db.backend == "sqlite":
                    query = query.replace("%s", "?").replace("ON CONFLICT (event_id) DO NOTHING", "OR IGNORE")
    ...
    def _map_row_to_event(self, row, cur) -> TrackingEvent:
        ...
        return TrackingEvent(
            ...
            custom_data=json.loads(data["custom_data"]) if isinstance(data["custom_data"], str) else data["custom_data"],
    ...
    async def save_emq_score(
        ...
        try:
            async with db.connection() as conn:
                ...
                cur.execute(query, (client_id, event_name, score, payload_size, has_pii))
        except Exception as e:
    ...
    async def list_by_visitor(self, external_id: ExternalId, limit: int = 100) -> List[TrackingEvent]:
        return []
    
    async def list_by_visitor_and_type(self, external_id: ExternalId, event_name: EventName, limit: int = 50) -> List[TrackingEvent]:
        return []

    async def list_by_date_range(self, start: datetime, end: datetime, event_name: Optional[EventName] = None, limit: int = 1000) -> List[TrackingEvent]:
        return []

    async def count_by_visitor(self, external_id: ExternalId) -> int:
        return 0

    async def count_by_type_and_date(self, event_name: EventName, date: datetime) -> int:
        return 0

...
def validate_phone(phone: Optional[str], default_country: str = "BO") -> PhoneValidationResult:
        ...
        if digits.startswith("591"):
            if len(digits) == 10:  # 591 + 7 digits
                return PhoneValidationResult(
                    is_valid=True, normalized=f"+{digits}", country_code="BO"
                )
        # If 7 digits, add 591
        elif len(digits) == 7:
            return PhoneValidationResult(
                is_valid=True, normalized=f"+591{digits}", country_code="BO"
            )
    ...
    if digits.startswith("1") and len(digits) == 11:  # US
        return PhoneValidationResult(is_valid=True, normalized=f"+{digits}", country_code="US")
...
def validate_url(
    url: Optional[str], allowed_schemes: Optional[list[str]] = None

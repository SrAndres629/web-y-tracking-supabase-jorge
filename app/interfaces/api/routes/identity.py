...
async def receive_google_credential(
    ...
    client_ip = request.headers.get("cf-connecting-ip") or (request.headers.get("x-forwarded-for") or "").split(",")[0].strip() or request.client.host
...
async def whatsapp_redirect(
    ...
    wa_url = await handler.handle(command) # This executes the tracking event in background internally.

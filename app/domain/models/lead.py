...
class Lead:
    ...
    def update_contact_info(
        ...
        if name:
            self.name = name.strip()[:100] or self.name
        if email:
            self.email = email
        self.updated_at = datetime.utcnow()
    ...
    def qualify(
        ...
        if pain_point:
            self.pain_point = pain_point[:200]
        if service_interest:
            self.service_interest = service_interest[:100]
        self.updated_at = datetime.utcnow()
    ...
    def __repr__(self) -> str:
        return f"Lead({self.id[:8]}, {self.phone}, {self.status.value})"
